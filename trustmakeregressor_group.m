...
for index=1:num_of_subjects
    ...
    %= ITI fixation (1), etc.
        ...
        for block= 1:4
            ...
            tmp_reg.(['regressors' num2str(block)]).to_censor = createSimpleRegressor(decision.event_beg,taskness.event_end, epoch_window, b.missed_trials(trial1_index:trial48_index));
            ...
            %to censor regressor for PE maps: last trial in experiment should
            %be censored 
            if block == 4
                tmp_reg.(['regressors' num2str(block)]).to_censor_PEmaps = createSimpleRegressor(decision.event_beg,taskness.event_end, epoch_window, [b.missed_trials(trial1_index:trial48_index-1); 1]);
            else
                tmp_reg.(['regressors' num2str(block)]).to_censor_PEmaps = createSimpleRegressor(decision.event_beg,taskness.event_end, epoch_window, b.missed_trials(trial1_index:trial48_index)); 
            end
...
%                
        % human vs non-human
        b.trustee_HC(strcmp(b.identity,'computer')) = -1;
...
%         
      ...
      [b.stim_times.trusteeBA_fsl,b.stim_times.trusteeBA_spmg]=write3Ddeconv_startTimes(data_dump_str,feedback.event_beg,feedback.event_end,'trusteeBA',b.trustee_HC,0);
        
       % trustee x trustee action interaction
      ...
      [b.stim_times.trusteeBYactionGB_fsl,b.stim_times.trusteeBYactionGB_spmg]=write3Ddeconv_startTimes(data_dump_str,feedback.event_beg,feedback.event_end,'GBxtaction',b.trusteeBYtactionGB,0);
       b.trusteeBYtactionBA = b.trustee_BA.*b.trustee_action;
      [b.stim_times.trusteeBYactionGB_fsl,b.stim_times.trusteeBYactionGB_spmg]=write3Ddeconv_startTimes(data_dump_str,feedback.event_beg,feedback.event_end,'BAxtaction',b.trusteeBYtactionBA,0);
