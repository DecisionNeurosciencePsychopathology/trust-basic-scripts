...
for index=1:num_of_subjects
    ...
    %         feedback.event_end(:,block) = b.outcome_OffsetTime(trial1_index:trial48_index)-firstfix_Onset+block_end*(block-1);
            ...
            %be censored (need to correct this part)
            if block == 4
                tmp_reg.(['regressors' num2str(block)]).to_censor_PEmaps = createSimpleRegressor(decision.event_beg,decision.event_end, epoch_window, [b.missed_trials(trial1_index:trial48_index-1); 1]);
            ...
            elseif any(b.missed_trials(trial1_index:trial48_index))==0
                tmp_reg.(['regressors' num2str(block)]).to_censor_PEmaps = zeros(size(createSimpleRegressor(decision.event_beg, decision.event_end, epoch_window))); 
            else
